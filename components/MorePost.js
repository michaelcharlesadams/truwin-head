
import Link from "next/link";

function MorePost({posts}) {
  
    console.log(posts);
    console.log(posts.slice(-2));

  return (
    <>
    
<div id="more-posts-section" className="lg:max-w-6xl mx-auto">
        <div id="more-posts-wrapper" className="mx-5 mt-20">
            <p className="text-2xl text-truwinblue-900 font-graphik">Keep browsing</p>
            <div id="more-posts-wrapper-content" className="grid sx:grid-cols-2 md:grid-cols-4 gap-4 mt-4 mx-auto">

                {posts.slice(-4).map((post) => 

                <div key={post.id} id="blog-post-card" className="relative my-4 w-full odd:bg-truwinsoftblue-primary even:bg-truwinblue-900 min-h-[370px] rounded">
                    <p className="absolute text-sm t-0 left-0 text-truwinblue-900 font-graphik pl-5 pt-5 ">{post.categories.nodes[0].name}</p>
                    <p className="absolute text-2xl t-0 left-0 text-truwinblue-900 font-graphik px-5 pt-20 md:text-lg">{post.title}</p>
                    <Link href={`/post${post.uri}`}> 
                    <a>
                        <button className="px-5 py-2 bg-truwinblue-700 text-white rounded-full absolute bottom-5 left-5">Read More</button>
                    </a>
                    </Link>
                    
                </div>
                    
                )}
               


            </div>
        </div>
    </div>



     <style jsx>{`
      #blog-post-card:nth-child(even) p {
        color: white;
    }
    `}
     </style>    
    
    
    </>
  )
}

export default MorePost