...
export async function getStaticProps(){
  ...
  //  Front Page Image for props.posts -  http://localhost:8888/graphql - process.env.HOST_DATA_LAYER
  /*
  const resPostImage = await fetch(process.env.HOST_DATA_LAYER, {
      method: 'POST',
  ...
  const jsonPI = await resPostImage.json()
  

  // Featured post 
  const resFeaturedPost = await fetch('http://localhost:8888/graphql', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
        query: `
  ...
  const jsonFP = await resFeaturedPost.json();
...
/*
  return {
    props: {
        posts: jsonPI.data.page,
        featuredPosts: jsonFP.data.posts.nodes,
    },
  }
  */
